...
Year* DbManager::yearGet(const long long int id)
{
	SqliteStatement* stm;
	Year* element;
	...
	{
		element = new Year(stm->getColumnInt64(0), stm->getColumnText(1));
	}
	else
	{
		element = NULL;
...
bool DbManager::yearGetAll(std::vector<Year* >& list)
{
	SqliteStatement* stm;
	std::vector<Year* >::iterator iter;
	int ret;
	Year* element;
